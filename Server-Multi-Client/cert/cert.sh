...
caCRT="../cert/ca/ca.crt"
caPEM="../cert/ca/ca.pem"
serverKey="../cert/server/server.key"
serverCSR="../cert/server/server.csr"
serverCRT="../cert/server/server.crt"
serverPEM="../cert/server/server.pem"
user1Key="../cert/user1/user1.key"
user1CSR="../cert/user1/user1.csr"
user1CRT="../cert/user1/user1.crt"
user1PEM="../cert/user1/user1.pem"
user2Key="../cert/user2/user2.key"
user2CSR="../cert/user2/user2.csr"
user2CRT="../cert/user2/user2.crt"
user2PEM="../cert/user2/user2.pem"
user3Key="../cert/user3/user3.key"
user3CSR="../cert/user3/user3.csr"
user3CRT="../cert/user3/user3.crt"
user3PEM="../cert/user3/user3.pem"
clientKey="../cert/client/client.key"
clientCSR="../cert/client/client.csr"
clientCRT="../cert/client/client.crt"
clientPEM="../cert/client/client.pem"
...
if [ ! -f "$user1CSR" ]; then  
    ...
    openssl req -new -key $user1Key -out $user1CSR -days 3650 -subj "/C=$country/ST=$state/L=$city/O=$organization/OU=$organizationalunit/CN=$clicommonname/emailAddress=$email"
fi
...
if [ ! -f "$user2CSR" ]; then  
    ...
    openssl req -new -key $user2Key -out $user2CSR -days 3650 -subj "/C=$country/ST=$state/L=$city/O=$organization/OU=$organizationalunit/CN=$clicommonname/emailAddress=$email"
fi
...
if [ ! -f "$user3CSR" ]; then  
    ...
    openssl req -new -key $user3Key -out $user3CSR -days 3650 -subj "/C=$country/ST=$state/L=$city/O=$organization/OU=$organizationalunit/CN=$clicommonname/emailAddress=$email"
fi
